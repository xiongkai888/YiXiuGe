 ...
 * 添加新群成员
 */
public class GroupSubAdapter extends ArrayAdapter<EMGroup> {

    private LayoutInflater inflater;
    private String newGroup;
    private boolean isStudent;//true学生false老师

    public GroupSubAdapter(Context context, int res, List<EMGroup> groups, boolean isStudent) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (getItemViewType(position) == 0) {
        ...
        } else if (!isStudent && getItemViewType(position) == 1) {
            if (convertView == null) {
            ...
            ((TextView) convertView.findViewById(R.id.name)).setText(getItem(position - (isStudent ? 1 : 2)).getGroupName());
        }
        return convertView;
    }
    ...
    @Override
    public int getCount() {
        return super.getCount() + (isStudent ? 1 : 2);
    }
