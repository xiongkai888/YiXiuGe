    ...
    protected void setEaseUIProviders() {
        ...
        easeUI.getNotifier().setNotificationInfoProvider(new EaseNotificationInfoProvider() {
            ...
            public String getDisplayedText(EMMessage message) {
                ...
                L.d(L.TAG,user.getNickname()+"2,2"+user.getUsername()+","+user.getAvatar());
                if (user != null) {
...
//        getUserBean(username,true,null);
    ...
    private YiXiuGeApi api = new YiXiuGeApi("app/getuser");

    public Map<String,UserBean> userBeanMap = new HashMap<>();//请求过的用户信息
    private Map<String,String> userIdMap = new HashMap<>();//请求过的用户id,防止重复加载

    private UserBeanEvent event = new UserBeanEvent();

    public UserBean getUserBeanByUid(String username){
    ...
    public void getUserBean(final String username,final UserInfoListener l) {
        ...
        api.addParams(CommonUtils.uid,uid);
        HttpClient.newInstance(appContext).request(api, new BeanRequest.SuccessListener<DataBean<UserBean>>() {
            @Override
